...
(async () => {
  ...
  for (const browserType of ['chromium']){//, 'firefox', 'webkit']) {
    ...
    await page.screenshot({path: './test2/pagina.png'})
    ...
    await page.screenshot({path:'./test2/logged in.png'})
    console.log('Successful logged in')
    
    await page.getByTitle("New post").click()
    await new Promise(r => setTimeout(r, 3000));
    await page.screenshot({path:'./test2/new post.png'})
    console.log('Creating a Post')


    await page.locator('[placeholder="Post title"]').fill('EP-010');
    ...
    await new Promise(r => setTimeout(r, 9000));
    await page.screenshot({path:'./test2/post_published.png'})
